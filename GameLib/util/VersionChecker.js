var VersionChecker = new function() {
	...
	this.remotePath = "https://raw.githubusercontent.com/Brian151/UnNamed-html5-Game-Library/master/GameLib/meta/version.json";
	this.localPath = "Gamelib/meta/version.json";
...
VersionChecker.init = function(onSuccess,onError) {
...
VersionChecker.showReleaseNotes = function(type) {
...
VersionChecker.checkForUpdate = function() {
	if (!this.hasLoadedJSONLocal || !this.hasLoadedJSONRemote) {
		var self = this;
		this.init(function(){
			self.checkForUpdate();
...
VersionChecker.displayCurrentBuildInfo = function(useFallback) {
	if (!useFallback) {
...
VersionChecker.showError = function(errorMsg) {
	console.error("Error : VersionChecker." + errorMsg);
}
VersionChecker.onXHRLoaded = function(onSuccess) {
	if (this.hasLoadedJSONLocal && this.hasLoadedJSONRemote) {
